...
export class FileScanner {
  static async scanProjectFolder(projectPath: string): Promise<EnvFile[]> {
    try {
      const entries: DirEntry[] = await invoke('read_directory', { path: projectPath });
  ...
  private static validateKeysAgainstExample(envFiles: EnvFile[], exampleFile: EnvFile): EnvFile[] {
    ...
    return envFiles.map(file => {
      ...
      const missingKeys = Array.from(exampleKeys).filter(key => !fileKeys.has(key));
      const extraKeys = Array.from(fileKeys).filter(key => !exampleKeys.has(key));
